...
public class ComplaintServiceResponse {
	// private fields
	 ...
	 */
	public ComplaintServiceResponse(String expectedUrl, String expectedMethod,
			String responseFile, int responseStatus) {
	...
	public void populateHttpResponse(HttpResponse response) throws IOException {
		...
		String responseBody = "";
		if (StringUtils.isEmpty(responseFile) == false) {
			// String filePath = FileUtils.combine("http_replies",
			// responseFile);
			String filePath = "http_replies/" + responseFile;
			System.out.println("populating response from file: " + filePath);
			ClassLoader classLoader = ComplaintServiceResponse.class
					.getClassLoader();
		...
		StringEntity entity = new StringEntity(responseBody,
