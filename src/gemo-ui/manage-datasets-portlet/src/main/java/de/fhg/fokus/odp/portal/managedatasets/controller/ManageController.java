	...
	public String submit() {
		...
		try {
			...
			if (metadataController.getMetadata().getType()
				...
				// storage
				try {
					Resource res4Sorage = metadataController.getMetadata()
							.newResource();
					res4Sorage.setUrl(CSVXMLFileUtils.uploadToStorage(
							uploadCSVXML.getUploadedFile()).toString());
					res4Sorage.setDescription("REST API entry point");
					res4Sorage.setFormat("rest");
				} catch (Exception e) {
					LOG.error("Error on upload csv/xml to storage: {}",
							uploadCSVXML.getUploadedFile(), e.getMessage());
				}
				...
				try {
					...
					r.setUrl(CSVXMLFileUtils.uploadToDocmentLibrary(
							uploadCSVXML.getUploadedFile()).toString());
					r.setDescription("CSV XML");
					final String ext = FilenameUtils.getExtension(uploadCSVXML
							.getUploadedFile().getName());
					r.setFormat(ext);
				} catch (Exception e) {
					LOG.error("Error csv/xml upload to document library: {}",
							uploadCSVXML.getUploadedFile(), e.getMessage());
				}
			}
			...
			if (metadataController.getMetadata().getType()
				...
				try {
					pathToWarFile = WarFileUtils.uploadToDocmentLibrary(
							uploadWar.getUploadedFile()).toString();
					LOG.debug("WarFileUtils added .war file to Liferay Document Library at:"
							+ pathToWarFile);
				} catch (IOException e) {
					LOG.error("Error WAR-Upload to document library: {}",
							uploadWar.getUploadedFile(), e.getMessage());
					e.printStackTrace();
				...
				// WAR -> AUTO-DEPLOYMENT
				try {
					WarFileUtils.uploadToAutoDeployService(uploadWar
							.getUploadedFile());
				} catch (Exception e) {
					LOG.error("Error WAR-Upload to document library: {}",
							uploadWar.getUploadedFile(), e.getMessage());
			...
			if (editMode
					|| !metadataController.getMetadata().getType()
							.equals(MetadataEnumType.APPLICATION)) {
				location += "/-/details/"
						+ odrClient.mungeTitleToName(metadataController
		...
		List<Resource> listOfResources = metadataController.getMetadata()
				.getResources();
		for (Iterator<Resource> iterator = listOfResources.iterator(); iterator
				.hasNext();) {
		...
		try {
			try {
				...
				// TODO: better errorhandling...
				LOG.error("Error on creating metadata: {}", metadataController
			...
			if (!editMode
				...
				try {
					...
					appOdrClient.persistMetadata(currentUser.getCurrentUser(),
							newApp);
