...
public class ODRClientTest {
	...
	public void extendedQueryMetadata() {
		...
		for (Metadata set : metadata) {
			log.info("metadata typ {}: {} (open: {})",
					new String[] { set.getType().toField(), set.getTitle(),
							String.valueOf(set.isOpen()) });
			for (Contact contact : set.getContacts()) {
				try {
					log.info("{}: {}", contact.getRole().getDisplayName(),
							contact.getName());
			...
			// log.info("published: {}",
			// set.getPublishedAsString("dd.MM.yyyy"));
	...
	public void createMetadata() {
		MetadataImpl impl = (MetadataImpl) odr
				.createMetadata(MetadataEnumType.DATASET);
		...
		try {
			impl.setTemporalCoverageFrom(DateFormat.getInstance().parse(
					"2012-06-01 00:00:00"));
			impl.setTemporalCoverageTo(DateFormat.getInstance().parse(
					"2013-06-01 00:00:00"));
	...
	public void getDataset() {
		try {
			...
			log.debug("metadata {} is of type {}", m.getName(), m.getType()
					.toField());
	...
	public void getApplication() {
		try {
			Metadata m = odr.getMetadata(null,
					"de-hh-wms-inspire-a1-6-flurstueck");
			log.debug("metadata {} is of type {}", m.getName(), m.getType()
					.toField());
	...
	public void listRelationships() {
		((ODRClientImpl) odr).listRelationships("de-hh-inspire-flurstueck",
				null);
	}
			...
			for (int i = 1; i < pages; i++) {
				...
				for (Metadata m : result.getResult()) {
					if (m.getLicence().getName() == null) {
						log.info("Found Metadata with null license: {} - {}",
								m.getName(), m.getTitle());
					} else if (m.getLicence().getName().isEmpty()) {
						log.info(
								"Found Metadata with empty license id: {} - {}",
								m.getName(), m.getTitle());
					}
				}
			}
		}
		log.info("fetched {} metadata in {} ms", count,
				System.currentTimeMillis() - start);
		System.out.println("fetched " + count + " metadata in "
	...
	public void getLatestDatasets() {
		...
		if (result.isSuccess()) {
			...
			log.info("number of results in the list of datasets(normal):"
					+ result.getResult().size());
			for (Dataset dataset : result.getResult()) {
				log.info("name - title: {} - {}", dataset.getName(),
						dataset.getTitle());
			}
		}
	}
	...
	@Test
	public void getLatestApplicationsAlternative() {
		Query query = new Query();
		ODRClientImpl odrcImpl = (ODRClientImpl) odr;
		query.getTypes().clear();
		query.getTypes().add(MetadataEnumType.APPLICATION);
		QueryResult<Application> result = odrcImpl.queryAlternative(query,
				Application.class);
		if (result.isSuccess()) {
			log.info("result.is success");
			log.info("number of results in the list of datasets(alternative):"
					+ result.getResult().size());
			for (Application dataset : result.getResult()) {
				log.info("name - title: {} - {}", dataset.getName(),
						dataset.getTitle());
			}
		}
	...
	@Test
	public void getLatestDatasetsAlternative() {
		Query query = new Query();
		ODRClientImpl odrcImpl = (ODRClientImpl) odr;
		query.getTypes().clear();
		query.getTypes().add(MetadataEnumType.DATASET);
		QueryResult<Dataset> result = odrcImpl.queryAlternative(query,
				Dataset.class);
		if (result.isSuccess()) {
			log.info("result.is success");
			log.info("number of results in the list of datasets(alternative):"
	...
	public void getLatestDocuments() {
		...
		if (result.isSuccess()) {
			for (Document document : result.getResult()) {
				log.info("name+title: {} - {}", document.getName(),
						document.getTitle());
			}
		}
	}
	...
	public void getLatestApplications() {
		...
		if (result.isSuccess()) {
			for (Application application : result.getResult()) {
				log.info("name+title: {} - {}", application.getName(),
