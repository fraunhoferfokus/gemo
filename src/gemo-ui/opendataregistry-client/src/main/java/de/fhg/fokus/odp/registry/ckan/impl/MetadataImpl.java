...
import static de.fhg.fokus.odp.registry.ckan.Constants.JSON_FIELD_SCOPES;
...
public class MetadataImpl implements Metadata, Serializable {
	...
	private void extractExtras() {
		...
		ExtraBean scopes = extras.remove(JSON_FIELD_SCOPES);
		if (scopes != null) {
			List<Scope> scopesList = ScopeImpl.read(extraToJson(scopes
					.getValue()));
			getScopes().addAll(scopesList);
		}
		...
		if (spatial != null) {
			JsonNode spatialNode = extraToJson(spatial.getValue());

			String type = spatialNode.get("type").getTextValue();
			SpatialDataImpl spatialDataImpl = null;
			try {
				spatialDataImpl = new SpatialDataImpl(
						SpatialEnumType.fromField(type.toLowerCase()));
			} catch (OpenDataRegistryException e) {
				e.printStackTrace();
			}

			JsonNode coordinatesNode = spatialNode.get("coordinates");
			if (coordinatesNode.isArray() && coordinatesNode.size() > 0) {
				JsonNode inner = coordinatesNode.get(0);
				for (JsonNode coordinate : inner) {
					if (coordinate.isArray()) {
						JsonNode xNode = coordinate.get(0);
						JsonNode yNode = coordinate.get(1);
						spatialDataImpl.addCoordinate(xNode.getDoubleValue(),
	...
	public JsonNode write() throws OpenDataRegistryException {
		...
		ArrayNode scopesNode = OM.createArrayNode();
		List<Scope> scopes = getScopes();
		for (Scope scope : getScopes()) {
			JsonNode sco = ((ScopeImpl) scope).write();
			scopesNode.add(sco);
		}

		if (scopesNode.size() > 0) {
			addExtras(extrasNode, JSON_FIELD_SCOPES, scopesNode);
		}
		...
		if (spatialData != null) {
			...
			ArrayNode coordinatesNode = OM.createArrayNode();
			for (Coordinate coordinate : spatialData.getCoordinates()) {
				ArrayNode coordinateNode = OM.createArrayNode();
				coordinateNode.add(coordinate.getX());
				coordinateNode.add(coordinate.getY());
				coordinatesNode.add(coordinateNode);
			}
			spatialDataNode.put("coordinates", coordinatesNode);
	...
	public SpatialData getSpatialData() {
		if (spatialData == null) {
			spatialData = new SpatialDataImpl(SpatialEnumType.POLYGON);
		}
		return spatialData;
	}
