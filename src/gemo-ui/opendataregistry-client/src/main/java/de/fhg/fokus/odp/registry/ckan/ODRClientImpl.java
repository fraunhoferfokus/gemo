...
import de.fhg.fokus.odp.registry.ckan.api.CKANClientActionAlternative;
...
//TODO replace deprecated methods with their jax-rs 2.0 alternatives 
public class ODRClientImpl implements ODRClient {
	...
	private CKANClientActionAlternative actionAlternative;
	...
	public void init(Properties props) {
		...
		URL ckanurlencoded;
		try {
			ckanurlencoded = new URL(base);
			url = ckanurlencoded.toString();
		} catch (MalformedURLException e) {
			log.debug("ckan url has illegal characters");
		}
		...
		action = ProxyFactory.create(CKANClientAction.class, url, executor);
		util = ProxyFactory.create(CKANClientUtil.class, url, executor);
		search = ProxyFactory.create(CKANClientSearch.class, url, executor);
		model = ProxyFactory.create(CKANClientModel.class, url, executor);
		// #################################################################
		// tested jax-rs 2.0 alternative

		// if connection poolsize is > 0, then ResteasyClientBuilder() defaults
		// to ThreadSafeClientConnManager()
		int connectionPoolSize = 10;
		ResteasyClient client = new ResteasyClientBuilder().connectionPoolSize(
				connectionPoolSize).build();
		ResteasyWebTarget target = client.target(url);
		actionAlternative = target.proxy(CKANClientActionAlternative.class);
		// TODO do this for other CKANClient classes to complete upgrade to
		// jaxrs 2.0
		// ##################################################################
	...
	public <T> QueryResult<T> queryAlternative(Query query, Class<T> clazz) {
		List<Metadata> all = new LinkedList<Metadata>();
		Response response = null;
		JsonNode param = queryToJson(query);

		response = actionAlternative.metadataSearchAlternative(param);
		...
		try {
			...
			if (response.getStatus() == 200) {
				JsonNode result = (JsonNode) response.getEntity();
	...
	public List<Category> listCategories() {
		...
		try {
			...
			if (response.getResponseStatus() == Status.OK) {
				JsonNode nodes = response.getEntity();
				...
				log.trace("REST < (http) returns: {} - {}", response
						.getResponseStatus().toString(), response
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#autoSuggestMetadata(java.lang.String)
	 */
	@Override
	public List<String> autoSuggestMetadata(String fragment) {
		...
		for (JsonNode node : list) {
			autocom.add(node.get("title").getTextValue());
		}
		return autocom;
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#autoSuggestTags(java.lang.String)
	 */
	@Override
	public List<String> autoSuggestTags(String fragment) {
		...
		List<String> autocom = new ArrayList<String>();
		for (JsonNode node : list) {
			autocom.add(node.get("Name").getTextValue());
		}
		return autocom;
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#autoSuggestFormats(java.lang.String)
	 */
	@Override
	public List<String> autoSuggestFormats(String fragment) {
		...
		List<String> autocom = new ArrayList<String>();
		for (JsonNode node : list) {
			autocom.add(node.get("Format").getTextValue());
		}
		return autocom;
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#queryMetadata(de.fhg.fokus.odp.registry
	 * .queries.Query)
	 */
	@Override
	public QueryResult<Metadata> queryMetadata(Query query) {
		return queryAlternative(query, Metadata.class);
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#queryDatasets(de.fhg.fokus.odp.registry
	 * .queries.Query)
	 */
	@Override
	public QueryResult<Dataset> queryDatasets(Query query) {
		...
		return queryAlternative(query, Dataset.class);
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#queryApplications(de.fhg.fokus.odp
	...
	public QueryResult<Application> queryApplications(Query query) {
		...
		return queryAlternative(query, Application.class);
	}
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#queryDocuments(de.fhg.fokus.odp.registry
	 * .queries.Query)
	 */
	@Override
	public QueryResult<Document> queryDocuments(Query query) {
		...
		return queryAlternative(query, Document.class);
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see de.fhg.fokus.odp.registry.ODRClient#getTagCounts()
	 */
	@Override
			...
			for (JsonNode node : result) {
				if (node.isArray()) {
					...
					for (JsonNode elem : node) {
						if (elem.isTextual()) {
							tag.setName(elem.getTextValue());
						} else if (elem.isInt()) {
							impl.setCount(elem.getLongValue());
						}
					}
					tags.add(impl);
				}
	...
	public User findUser(String name) {
		...
		try {
			...
			if (response.getStatus() == Response.Status.OK.getStatusCode()) {
				JsonNode node = response.getEntity();
				log.trace("REST < returns: {}", node);

				JsonNode userNode = node.get("result");

				impl = new UserImpl(convert(userNode, UserBean.class), this);
				JsonNode packages = userNode.get("datasets");
				if (packages != null && packages.isArray()) {
					for (JsonNode package_ : packages) {
						JsonNode packageName = package_.get("name");
						if (packageName != null && packageName.isTextual()) {
							impl.getDatasets().add(packageName.getTextValue());
						}
					}
				}
			} else {
				log.trace("REST < (http) returns: {} - {}", response
						.getResponseStatus().name(), response
						.getResponseStatus().getReasonPhrase());
			}
		} finally {
			if (response != null) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.fhg.fokus.odp.registry.ODRClient#createUser(java.lang.String,
	...
	public User createUser(String name, String email, String password) {
		...
		try {
			...
			log.debug("/api/3/action/user_create: {}ms",
					System.currentTimeMillis() - start);
			if (response.getStatus() == Response.Status.OK.getStatusCode()) {
				JsonNode result = response.getEntity();
				log.trace("REST < returns: {}", response);
	...
	private <T> QueryResult<T> query(Query query, Class<T> clazz) {
		...
		try {
			...
			if (response.getResponseStatus() == Status.OK) {
				...
				log.debug("REST < (http) returns: {} - {}", response
						.getResponseStatus().getStatusCode(), response
						.getResponseStatus().getReasonPhrase());
				queryResult.setSuccess(false);
				queryResult.setErrorMessage(response.getResponseStatus()
						.getReasonPhrase());
			}
		} finally {
			if (response != null) {
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#createMetadata(de.fhg.fokus.odp.registry
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#rateMetadata(de.fhg.fokus.odp.registry
	 * .model.User, java.lang.String, int)
	 */
	@Override
	public void rateMetadata(User user, String metadata, int rate) {
	 ...
	 * 
	 * @see
	 * de.fhg.fokus.odp.registry.ODRClient#showRoles(de.fhg.fokus.odp.registry
	...
	public List<String> showRoles(User user, String object) {
		...
		if (isSuccess(node)) {
			...
			for (JsonNode entry : rolesNode) {
				roles.add(entry.get("role").getTextValue());
			}
		}
		return roles;
	}
	...
	public void updateRoles(User user, String object, List<String> roles) {
		...
		log.debug("/api/3/action/user_role_update: {}ms",
				System.currentTimeMillis() - start);
		log.trace("REST < returns: {}", node);
	}
	...
	private int getCount(JsonNode node) {
		JsonNode result = node.get("result");
		if (result != null) {
			return result.get("count").getIntValue();
		}
		return 0;
	}
	...
	public void persistMetadata(User user, Metadata metadata)
		...
		String auth = user == null ? authorizationKey : ((UserImpl) user)
				.getApikey();
		...
		if (impl.isNew()) {
			...
			log.trace("REST > calling action api 'package_update' with: {}",
					node);
			long start = System.currentTimeMillis();
			JsonNode result = action.updateMetadata(auth, node);
			log.debug("/api/3/action/package_update: {}ms",
					System.currentTimeMillis() - start);
			log.trace("REST < returns: {}", result);
		}
	}
	...
	public Metadata getMetadata(User user, String name)
		...
		ObjectNode body = OM.createObjectNode();
		body.put("id", name);
		String key = user != null ? ((UserImpl) user).getApikey()
				: authorizationKey;

		log.trace("REST > calling action api 'package_show' with: {}", body);
		long start = System.currentTimeMillis();
		JsonNode node = action.showMetadata(key, body);
	...
	public void loadRating(Metadata metadata) {
		...
		log.trace("REST > calling model dataset api with: {}",
				metadata.getName());
		long start = System.currentTimeMillis();
		JsonNode node = model.getDataset(metadata.getName());
		...
		if (count != null && count.isNumber()) {
			((MetadataImpl) metadata).setRatingCount(count.getNumberValue()
					.intValue());
		}
		JsonNode average = node.get("ratings_average");
		if (average != null && average.isDouble()) {
			((MetadataImpl) metadata).setAverageRating(average.getNumberValue()
					.intValue());
		}
	}
